...
public class InstService {
    ...
    private static String getLocalIp() {
        ...
        throw new RuntimeException(
                "Cannot determine local non-loopback IP. Please configure auth_realm explicitly on the instance.");
    }
    ...
    public void writeHtpasswd(Inst inst) {
        ...
        File htpasswd_file = new File(StringUtils.joinWith(
                File.separator, homeDir.getHomeAbsolutePath(), "registry", inst_name, "config", "htpasswd"));
        try {
        ...
        if (accountList != null && !accountList.isEmpty()) {
            ...
            log.info(
                    "Written htpasswd file with {} accounts from Account table for instance {}",
                    lines.size(),
                    inst_name);
        } else {
            ...
            if (htpasswdList != null) {
                ...
                log.info(
                        "Written htpasswd file with {} users from Htpasswd table for instance {}",
                        lines.size(),
                        inst_name);
            }
        }
    ...
    public void generateKeyPairForInst(Inst inst, String keyType, String algorithm) {
        try {
            ...
            log.info(
                    "Generated {} key pair for instance {} with algorithm {}",
                    keyPair.getKeyType(),
                    inst.getName(),
    ...
    private void writeJwksFile(Inst inst, String jwksJson) {
        File jwksFile = new File(StringUtils.joinWith(
                File.separator, homeDir.getHomeAbsolutePath(), "registry", inst.getName(), "config", "jwks.json"));
        try {
    ...
    public void runGarbageCollection(Inst inst) {
        ...
        String configPath = StringUtils.joinWith(
                File.separator, homeDir.getHomeAbsolutePath(), "registry", instName, "config", "config.yml");
