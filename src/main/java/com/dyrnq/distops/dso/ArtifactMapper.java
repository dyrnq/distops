...
public class ArtifactMapper extends BaseMapperWrap<Artifact> {
    ...
    public Artifact findByInstIdAndRepoIdAndTagName(Long instId, Long repoId, String tagName) {
        return this.selectItem(c -> {
            c.whereEq(Artifact.INST_ID, instId).andEq(Artifact.REPO_ID, repoId).andEq(Artifact.TAG_NAME, tagName);
        });
    }
    ...
    public Integer countDistinctArtifactsByRepoId(Long repoId) {
        try {
            long count = db().sql(
                            "SELECT COUNT(DISTINCT tag_name) FROM artifact WHERE repo_id = ? AND tag_name IS NOT NULL",
                            repoId)
                    .getCount();
            return (int) count;
        } catch (Exception e) {
